...
- transform_points_to_mm(), parallax_factor(), measure_length_mm()
                          : ChArUco 호모그래피 기반 mm 변환 (v2)
- MEASUREMENTS_CSV_HEADER, append_csv_rows()
                          : measurements.csv 등에 공통으로 쓰는 CSV 저장 형식
...
# measurements.csv에 공통으로 쓰는 컬럼 (수동/자동 측정 모두 동일 포맷 사용)
...
# CSV 저장 공용 함수
# ---------------------------------------------------------------------------
def append_csv_rows(csv_path, rows, header):
    """
    rows(dict 리스트)를 csv_path에 이어 쓴다. 파일이 없으면 헤더를 먼저 쓰고,
    상위 폴더가 없으면 자동으로 만든다.
    """
    ensure_dir(csv_path)
