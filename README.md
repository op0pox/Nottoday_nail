...
손톱 세그멘테이션 학습과 실험 산출물입니다. 이미지 데이터 대부분은 `.gitignore`로 제외하고, **가중치 `weights/*.pt`와 `train.py` / `test.py`만** 커밋합니다.

| 경로 | 역할 |
|---|---|
| `train.py` | HuggingFace 사전학습 모델로 YOLO 학습. 기본 데이터는 `TrainDataset/YOLODataset_white` |
| `test.py` | 두 모델 예측을 GT와 나란히 비교해 `results/`에 저장 |
